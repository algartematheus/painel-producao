...
import { Sun, Moon, PlusCircle, Package, List, Edit, Trash2, Save, XCircle, ChevronLeft, ChevronRight, MessageSquare, Layers, ChevronUp, ChevronDown, LogOut, Eye, EyeOff, ChevronDown as ChevronDownIcon, Monitor, X, Settings, GripVertical } from 'lucide-react';
...
// --- NOVA FUNCIONALIDADE: MODO TV ---
const TVDashboardView = ({ dashboardName, processedData, summary, onExit }) => {
    const latestSummary = summary || { totalProduced: 0, totalGoal: 0, averageEfficiency: 0 };
    const totalPeople = processedData.length > 0 ? processedData[0].people : 0;
    const totalMinutes = processedData.reduce((acc, item) => acc + (item.people * item.availableTime), 0);

    return (
        <div className="fixed inset-0 bg-gray-900 text-white p-8 flex flex-col font-sans">
            <header className="flex justify-between items-center border-b-2 border-gray-700 pb-4">
                <h1 className="text-6xl font-bold tracking-wider">{dashboardName}</h1>
                <div className="text-right">
                    <h2 className="text-4xl font-semibold">{new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</h2>
                    <h3 className="text-3xl text-gray-400">{new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</h3>
                </div>
                <button onClick={onExit} className="absolute top-8 right-8 text-gray-500 hover:text-white transition-colors">
                    <X size={40} />
                </button>
            </header>
            ...
            <section className="grid grid-cols-3 gap-8 my-8">
                <div className="bg-gray-800 p-6 rounded-lg text-center">
                    <h4 className="text-3xl text-gray-400 uppercase tracking-widest">Nº de Pessoas</h4>
                    <p className="text-8xl font-bold">{totalPeople}</p>
                </div>
                <div className="bg-gray-800 p-6 rounded-lg text-center">
                    <h4 className="text-3xl text-gray-400 uppercase tracking-widest">Minutos Disponíveis</h4>
                    <p className="text-8xl font-bold">{totalMinutes.toLocaleString('pt-BR')}</p>
                </div>
                <div className="bg-gray-800 p-6 rounded-lg text-center">
                    <h4 className="text-3xl text-gray-400 uppercase tracking-widest">Eficiência do Dia</h4>
                    <p className={`text-8xl font-bold ${latestSummary.averageEfficiency < 65 ? 'text-red-500' : 'text-green-400'}`}>{latestSummary.averageEfficiency}%</p>
                </div>
            </section>

            <main className="flex-1 bg-gray-800 rounded-lg p-6 overflow-hidden">
                <table className="w-full h-full text-center">
                    <thead className="text-3xl text-gray-400 uppercase">
                        <tr>
                            <th className="w-1/6">Hora</th>
                            {processedData.map(d => <th key={d.id}>{d.period}</th>)}
                        </tr>
                    </thead>
                    <tbody className="text-6xl font-bold divide-y divide-gray-700">
                        <tr className="h-1/4"><td className="text-left text-3xl text-gray-400 uppercase">Meta</td>{processedData.map(d => <td key={d.id}>{d.goal}</td>)}</tr>
                        <tr className="h-1/4"><td className="text-left text-3xl text-gray-400 uppercase">Produção</td>{processedData.map(d => <td key={d.id}>{d.produced}</td>)}</tr>
                        <tr className="h-1/4"><td className="text-left text-3xl text-gray-400 uppercase">Eficiência</td>{processedData.map(d => <td key={d.id} className={`${d.efficiency < 65 ? 'text-red-500' : 'text-green-400'}`}>{d.efficiency}%</td>)}</tr>
                        <tr className="h-1/4 border-t-4 border-blue-500"><td className="text-left text-3xl text-gray-400 uppercase">Meta Acum.</td>{processedData.map(d => <td key={d.id}>{d.cumulativeGoal}</td>)}</tr>
                        <tr className="h-1/4"><td className="text-left text-3xl text-gray-400 uppercase">Prod. Acum.</td>{processedData.map(d => <td key={d.id}>{d.cumulativeProduction}</td>)}</tr>
                        <tr className="h-1/4"><td className="text-left text-3xl text-gray-400 uppercase">Efic. Acum.</td>{processedData.map(d => <td key={d.id} className={`${d.cumulativeEfficiency < 65 ? 'text-red-500' : 'text-green-400'}`}>{d.cumulativeEfficiency}%</td>)}</tr>
                    </tbody>
                </table>
            </main>
        </div>
...
// --- NOVO COMPONENTE: PAINEL DE CONFIGURAÇÕES DO MODO TV ---
const TVSettingsModal = ({ isOpen, onClose, settings, onSave }) => {
    ...
    const onDragStart = (e, index) => {
        setDraggedItem(localOrder[index]);
        e.dataTransfer.effectAllowed = 'move';
    };

    const onDragOver = (index) => {
        const draggedOverItem = localOrder[index];
        if (draggedItem === draggedOverItem) {
            return;
        }
        const items = localOrder.filter(item => item !== draggedItem);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-2xl w-full max-w-lg">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold">Configurações do Modo TV</h2>
                    <button onClick={onClose}><XCircle /></button>
                </div>
                
                <div className="space-y-6">
                    <div>
                        <label htmlFor="tvInterval" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Tempo de exibição de cada quadro (em segundos)
                        </label>
                        <input
                            type="number"
                            id="tvInterval"
                            ...
                            {localOrder.map((dashboardId, index) => {
                                ...
                                return (
                                    <li
                                        ...
                                        onDragEnd={onDragEnd}
                                        className="flex items-center p-3 cursor-move border-b dark:border-gray-700 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-800"
                                    >
                ...
                <div className="mt-8 flex justify-end">
                    <button onClick={handleSave} className="h-10 px-6 font-semibold rounded-md bg-blue-600 text-white hover:bg-blue-700">
...
const CronoanaliseDashboard = ({ user }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 dark:bg-black text-gray-800 dark:text-gray-200 font-sans">
            <TVSettingsModal 
                isOpen={isTvSettingsOpen} 
                onClose={() => setIsTvSettingsOpen(false)} 
                settings={tvSettings} 
                onSave={handleSaveTvSettings} 
            />
